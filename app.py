...
def menu():
    ...
    while (user_input := input(MENU_PROMPT)) != '9':
        if user_input == '1':
            ...
            prompt_see_all_glosses(gloss_db)
        elif user_input == '3':
...
def prompt_see_all_glosses(gloss_db):
...
def prompt_add_new_gloss(gloss_db):
    ...
    if not suppliers:
        print("No suppliers exist. Add suppliers first.")
        return
...
def get_supplier_info(supplier_db, gloss_db):
    ...
    gloss = gloss_db.get_best_prep_for_gloss(gloss_name)

    print("Gloss returned:", gloss)
    ...
    supplier_id = gloss[5]
    print("Supplier ID from gloss:", supplier_id)

    supplier = supplier_db.get_suppliers_by_id(supplier_id)
    print("Supplier lookup result:", supplier)

    if not supplier:
        print("No supplier found for this gloss.")
    else:
